...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
        ...
        $$('.form-textarea-limit-indicator span').each(function (el) {
            ...
            var countText = function (firstRun) {
                ...
                var limitByType = function (type) {
                    ...
                    if (limitType == 'Words') {
                        count = $A(cleaned_contents.split(/\s+\b/)).without("").length;
    ...
    checkCalculation: function (calc) {
        ...
        if (!(typeof output== "string" && output.length > 0) && parseFloat(output) === 0 && $('input_' + result) && ($('input_' + result).readAttribute('defaultValue') != null || $('input_' + result).readAttribute('data-defaultvalue') != null)) {
